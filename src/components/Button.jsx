...
const Button = React.forwardRef(({ text, className, onClick }, ref) => {
  return (
    <button 
      ref={ref}
      onClick={onClick}
      className={`${className ?? ''} group relative inline-flex items-center gap-2 px-6 py-3 rounded-full border-2 border-white/30 text-white font-medium text-sm md:text-base overflow-hidden transition-all duration-300 hover:border-orange-100 backdrop-blur-sm bg-white/5`}
    >
